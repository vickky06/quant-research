...
# Verdict is a contractual record — stored at workspace root under docs/heldout/
# so it's tracked in git and outlives any prototype directory.
_WORKSPACE_ROOT = Path(__file__).parent.parent
VERDICT_DIR = _WORKSPACE_ROOT / "docs" / "heldout"
...
def main() -> int:
    ...
    # Data start must extend backward (13m for momentum + regime warmup)
    ho_data_start = pd.Timestamp("2022-06-01")  # ~18m before held-out start
    symbols = get_universe(fast=False)
    print(f"[step] universe: {len(symbols)} symbols")

    download_index(db_path, ho_data_start, HELDOUT_END)
    download_prices(symbols, ho_data_start, HELDOUT_END, db_path)
