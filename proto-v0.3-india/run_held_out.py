"""One-shot held-out validation harness — Gate G3 (India v0.3).
...
Strategy evaluated:
    meta_ensemble with regime-conditional signal weights (passed G2):
    - mean_reversion + sector_neutral_mr + momentum on Nifty 500
    - DOWN_TREND regimes: MR-heavy (45/45/10)
    - UP_TREND regimes:   momentum-heavy (15/20/65)
    - No hard regime gate (India LOW_VOL_UP_TREND IC is positive)

Enforcement:
    1. Script REFUSES to run without --confirm-final-evaluation.
    2. After a successful run, writes verdict files (read-only markers).
    3. Subsequent invocations detect markers and REFUSE to re-run unless
       --i-am-consciously-violating-the-contract is passed.
...
HELDOUT_END = pd.Timestamp("2026-06-30")  # Contract v1.3 §3

_WORKSPACE_ROOT = Path(__file__).parent.parent
VERDICT_DIR = _WORKSPACE_ROOT / "docs" / "heldout-india-v03"
VERDICT_MD = VERDICT_DIR / "verdict.md"
VERDICT_JSON = VERDICT_DIR / "verdict.json"
OVERRIDE_MD = _WORKSPACE_ROOT / "HELDOUT_OVERRIDE_INDIA_V03.md"

# Locked strategy config — must match G2-passing run exactly.
...
def check_lock() -> None:
    if VERDICT_JSON.exists() or VERDICT_MD.exists():
        ...
        print("  1. Write your justification in HELDOUT_OVERRIDE_INDIA_V03.md")
...
def load_last_training_dsr() -> float:
    ...
    runs = read_runs()
    if runs.empty:
        print("[fatal] no run_log entries. Execute `python run.py` first.")
        sys.exit(3)
...
def main() -> int:
    ...
    print("=" * 70)
    print("Gate G3 — One-shot held-out evaluation (India v0.3)")
    print("=" * 70)
    ...
    print(f"Strategy: regime-conditional meta_ensemble")
    print(f"  DOWN_TREND: MR=45%, sector_mr=45%, momentum=10%")
    print(f"  UP_TREND:   MR=15%, sector_mr=20%, momentum=65%")
    print("=" * 70)
    ...
    md_lines = [
        f"# Held-out Verdict — Gate G3 (India v0.3)",
        f"",
        f"**Verdict**: **{verdict}**",
        f"**Timestamp**: {verdict_payload['timestamp']}",
        f"**Contract version**: 1.3",
        f"**Held-out window**: {HELD_OUT_START.date()} → {HELDOUT_END.date()}",
        f"",
        f"## Strategy",
        f"- Signals: mean_reversion, sector_neutral_mr, momentum",
        f"- Regime weights: DOWN_TREND → MR-heavy; UP_TREND → momentum-heavy",
        f"- No hard regime gate",
        f"",
        f"## Metrics",
    ...
    md_lines.extend([
        ...
        f"*override_used = {args.i_am_consciously_violating_the_contract}*",
    ])
    ...
    log_run(
        ...
        notes="Gate G3 held-out one-shot evaluation — India v0.3",
    )
