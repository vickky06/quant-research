"""v0.3 R2K pipeline runner — S&P SmallCap 600, momentum + mean_reversion.

Evaluates both Agents independently (Gate G1) and the IC-weighted
Meta-Ensemble (Gate G1 + Gate G2 hint). Walk-forward validation + multi-trial
deflated Sharpe bracket follow G1.

Usage:
    python run.py                        # all agents + ensemble, full S&P 600
    python run.py --fast                 # fallback 50-ticker list
    python run.py --agent momentum       # single agent
    python run.py --agent mean_reversion
    python run.py --agent meta_ensemble
"""
...
def run_meta_ensemble(
    ...
    result = P.run_meta_ensemble_backtest(
        ...
        skip_regimes=None,  # evaluate unconditionally on R2K first
    )
    ...
    for a in weights_df.columns:
        ...
        print(f"    {a:20s}  mean={w_series.mean():.2%}  min={w_series.min():.2%}  max={w_series.max():.2%}")
...
def run_for_signal(
    ...
    print(f"  Max drawdown:      {dd_result['max_drawdown']:+.2%}  "
          ...
          f"recovery: {dd_result.get('recovery_months')}m)")
    insufficient = set(gate.get("regimes_insufficient_sample", []))
    print(f"  Regime Scorecard (n<{gate.get('regime_min_n', 10)} excluded per Contract v1.3):")
    for regime, row in scorecard.iterrows():
        n = int(row["count"])
        excluded = regime in insufficient
        if excluded:
...
def main(fast: bool = False, agent: str = "all") -> int:
    ...
    print("=" * 70)
    print("v0.3 R2K — S&P SmallCap 600: momentum + mean_reversion")
    ...
    print(f"Index proxy:     {P.INDEX_TICKER} (Russell 2000)")
    print(f"Cost model:      {P.ROUNDTRIP_COST_BPS} bps roundtrip")
    print(f"Mode: {'FAST (50 fallback tickers)' if fast else 'FULL (S&P 600)'}")
    ...
    for target in targets:
        if target == "ensemble":
            results.append(
                run_for_signal(
                    target, ensemble_signal, close_wide, liquidity_60d, regime_df,
                    mode, len(symbols)
                )
            )
            ...
            results.append(
                run_for_signal(
                    target, signals[target], close_wide, liquidity_60d, regime_df,
                    mode, len(symbols)
                )
            )
    ...
    print("=" * 70)
    header = (
        f"{'Agent':<20} {'G1':<6} {'DSR':>6} {'Sharpe':>8} {'CumRet':>9} {'MaxDD':>8}"
    )
    print(header)
    print("-" * len(header))
    for r in results:
        print(
            f"{r['agent']:<20} {r['verdict']:<6} {r['dsr']:>6.3f} "
    ...
    summary_lines = [
        "v0.3 R2K Backtest Summary — S&P SmallCap 600",
        "=" * 70,
        f"Training window: {TRAINING_START.date()} → {TRAINING_END.date()}",
        f"Index proxy: {P.INDEX_TICKER} (Russell 2000)",
        f"Mode: {mode}",
    ...
    for r in results:
        summary_lines.append(
            f"{r['agent']:<20} {r['verdict']:<8} {r['dsr']:>6.3f} "
            f"{r['sharpe']:>+8.3f} {r['cum_return']:>+9.2%}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fast", action="store_true", help="50 fallback tickers only")
    parser.add_argument(
        "--agent",
        default="all",
        help="momentum | mean_reversion | ensemble | meta_ensemble | all (default)",
    )
